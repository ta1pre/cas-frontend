...
interface BasicPostProps {
  targetType: string;
  orderIndex: number;
  onUploadComplete: () => void; // ✅ 追加
}

const BasicPost = ({ targetType, orderIndex, onUploadComplete }: BasicPostProps) => {
  const user = useUser();
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      onUploadComplete(); // ✅ アップロード後に `onUploadComplete` を呼び出す
    } catch (error) {
  ...
  const handleDeleteClick = async () => {
    ...
    try {
      ...
      onUploadComplete(); // ✅ 削除後にも `onUploadComplete` を呼び出す
    } catch (error) {
  ...
  return (
    <div className="relative w-48 h-64 border-2 border-dashed border-gray-400 flex items-center justify-center mb-4 rounded-lg overflow-hidden">
      {uploadedUrl ? (
        <>
          <img src={uploadedUrl} alt={`${targetType}画像 ${orderIndex + 1}`} className="w-full h-full object-cover" />
          <button
